...
//留言内容recyclerview列表控件适配器
public class LiuYanAdapter extends RecyclerView.Adapter<LiuYanAdapter.VH> {
    private final Context context;
    private List<ContentAndImgBean> mBeanList;

    //构造方法 初始化数据
    public LiuYanAdapter(Context context, List<ContentAndImgBean> beanList) {
    ...
    public VH onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_liuyan, parent, false);
        return new VH(v);
    }
    ...
    public void onBindViewHolder(final VH holder, @SuppressLint("RecyclerView") int position) {
        ...
        if (!"".equals(mBeanList.get(position).getContent())) {
            holder.tv_content.setVisibility(View.VISIBLE);
            holder.tv_content.setText(mBeanList.get(position).getContent());
        } else {
