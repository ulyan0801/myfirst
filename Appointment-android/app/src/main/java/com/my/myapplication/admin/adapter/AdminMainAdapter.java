...
public class AdminMainAdapter extends RecyclerView.Adapter<AdminMainAdapter.VH> {
    ...
    //构造方法 初始化数据
    public AdminMainAdapter(Context context, List<AppointmentBean> beanList) {
    ...
    public void onBindViewHolder(final VH holder, @SuppressLint("RecyclerView") int position) {
        ...
        if ("".equals(xingqi)) {
            holder.tv_zhouji.setText("周几开放预约: 未开放预约");

        } else {
            holder.tv_zhouji.setText("周几开放预约: " + xingqi.replace(",", " "));
